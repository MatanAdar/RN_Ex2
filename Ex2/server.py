...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # * Fill in start (1)
        server_socket.bind((host, port))  #binds the server ip and port number to the socket
        server_socket.listen(1)  #determines the number of connections at the same time and listen for incoming connections
        # * Fill in end (1)
        ...
        while True:
            try:
                ...
                # * Fill in start (2)
                # server is waiting for the client connection request
                # and then create connection socket between the client and the server
