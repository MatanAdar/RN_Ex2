...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # * Fill in start (1)
        proxy_socket.bind(proxy_address)  #binds a tuple of proxy ip and port number (that call proxy_address), to the socket
        proxy_socket.listen(1)  #determines the number of connections at the same time and listen for incoming connections
        # * Fill in end (1)
        ...
        while True:
            try:
                ...
                # * Fill in start (2)
                # proxy is waiting for the client connection request
                # and then create connection socket between the client and the proxy
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int],
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            # * Fill in start (3)
            # data contains the package that the client sent according to the determined buffer size
            ...
            try:
                ...
                # * Fill in start (4)
                client_socket.sendall(response)  # sending the response back to the client
                # * Fill in end (4)
